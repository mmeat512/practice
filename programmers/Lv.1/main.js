(() => {
  ...
  function solution27(board, moves) {
    ...
    for (let i = 0; i < board.length; i++) {
      targetList.push(board.map((item) => item[i]));
    }

    for (let i = 0; i < moves.length; i++) {
      const target = targetList[moves[i] - 1];
      for (let j = 0; j < target.length; j++) {
        if (target[j] !== 0) {
          const f = result[result.length - 1];
          if (f === target[j]) {
            result.pop();
            answer += 2;
          } else {
  ...
  function solution30(survey, choices) {
    ...
    return objList.reduce((acc, item) => {
      const values = Object.values(item);
      // 더 높은 values를 갖고 있는 키 값을 반환
      return (acc += Object.keys(item)[values.indexOf(Math.max(...values))]);
    }, '');
  }
  ...
  function solution33N(players, callings) {
    let maps = {};
  ...
  // 공원산책
  function solution34(park, routes) {
    var answer = [];
    const wEnd = park[0].length - 1;
